...
def worker(work_path,
    ...
    if('phonolammps' not in command and not inter_type in ['vasp','abacus']):
        command = "/root/local/deepmd-kit-2.1.0/bin/phonolammps in.lammps -c POSCAR --dim %s %s %s && /root/local/deepmd-kit-2.1.0/bin/lmp -i in.lammps"%(supercell_matrix[0],supercell_matrix[1],supercell_matrix[2])
