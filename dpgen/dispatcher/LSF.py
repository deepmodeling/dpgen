...
class LSF(Batch):
    ...
    def check_status(self):
        ...
        if job_id == "":
            raise RuntimeError(
                "job %s is has not been submitted" % self.context.remote_root)
    ...
    def _check_sub_limit(self, task_max, **kwarg):
        stdin_run, stdout_run, stderr_run = self.context.block_checkcall(
            "bjobs | grep RUN | wc -l")
        njobs_run = int(stdout_run.read().decode('utf-8').split('\n')[0])
        stdin_pend, stdout_pend, stderr_pend = self.context.block_checkcall(
