...
def run_md_model_devi(iter_index, jdata, mdata):
    ...
    if model_devi_engine == "lammps":
        ...
        if nbeads is None:
            ...
            command = f"{{ all_exist=true; for i in $(seq -w 1 {nbeads}); do [[ ! -f "dpgen.restart${i}.1000" ]] && { all_exist=false; break; }; done; $all_exist && { {model_devi_exec} -p {nbeads}x1 -i input.lammps -v restart 0; } || { {model_devi_exec} -p {nbeads}x1 -i input.lammps -v restart 1; } }}"
        command = "/bin/sh -c '%s'" % command
        commands = [command]
