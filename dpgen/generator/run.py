...
def _link_fp_abacus_pporb_descript(iter_index, jdata):
    ...
    for ii in fp_tasks:
        ...
        if fp_dpks_model != None:
            ...
            assert(os.path.isfile(model_file)),"Can not find the deepks model file %s, which is defined in %s/INPUT" % (model_file,ii)
        ...
        for jj in pp_files:
            ...
            assert(os.path.isfile(ifile)),"Can not find the pseudopotential file %s, which is defined in %s/STRU" % (ifile,ii)
            os.symlink(ifile, jj)
        
        for jj in orb_files:
            ifile = os.path.join(fp_pp_path, jj)
            assert(os.path.isfile(ifile)),"Can not find the orbital file %s, which is defined in %s/STRU" % (ifile,ii)
            os.symlink(ifile, jj)
            
        if descriptor_file != None:
            ifile = os.path.join(fp_pp_path, descriptor_file)
            assert(os.path.isfile(ifile)),"Can not find the deepks descriptor file %s, which is defined in %s/STRU" % (ifile,ii)
